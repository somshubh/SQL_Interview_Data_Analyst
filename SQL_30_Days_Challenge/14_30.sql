...
with cte as(
select customer_id,	SUM(total_items_ordered) total_order, SUM(returned_items) total_return,
round((cast(SUM(returned_items) AS float) / SUM(total_items_ordered))*100, 2) as Percentage
from orders o join returns r on o.order_id = r.order_id
group by customer_id
)
select Top 5 customer_id from cte order by Percentage desc;
