...
-- select * from zomato_orders where YEAR(order_date) = '2023' and MONTH(order_date) = 11;

with cte as(
select *,
COUNT(customer_id) over(partition by city, customer_id) order_cont
from zomato_orders where YEAR(order_date) = '2023' and MONTH(order_date) = 11
),
cte2 as(
select distinct city, customer_id, order_cont from cte where order_cont > 3
)
select city, COUNT(customer_id) as customer_count from cte2 group by city
