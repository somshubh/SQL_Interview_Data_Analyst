...
with cte as(
select *,
COUNT(*) over(PARTITION by company_id, cast(title as varchar), cast(description as varchar)) cnt
from job_listings
)
select COUNT(distinct company_id) as duplicate_comp_count  from cte where cnt > 1