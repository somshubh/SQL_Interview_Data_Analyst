...
with cte as(
select *,
row_number() over(partition by departmentId order by salary desc) as flag
from Employee
)
select d.name department_Name, c.salary from cte c join Department d on d.id = c.departmentId where flag <= 3;