...
with cte as(
select *,
ROW_NUMBER() over(PARTITION BY user_id order by (likes + comments) desc) as flag,
likes + comments as total
from fb_posts
)
select user_id, post_id, total as engagement_count, flag as rank from cte where flag <=3