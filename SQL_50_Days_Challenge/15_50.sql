...
select
round(cast(DATENAME(HOUR,order_time) as float) + cast(DATENAME(MINUTE,order_time) as float)/60,0) as hour,
COUNT(order_id) as total_orders,
COUNT(2)/cast((select COUNT(*) from order_data) AS float)*100 as Order_Percentage
from order_data
group by round(cast(DATENAME(HOUR,order_time) as float) + cast(DATENAME(MINUTE,order_time) as float)/60,0)
order by hour;
