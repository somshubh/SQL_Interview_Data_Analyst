...
--select COUNT(*) - (select COUNT(*) from walmart_eu where customerid is null) from walmart_eu;

--select count(distinct customerid) from walmart_eu;
...
select customerid, description, Purchase_Count from cte where rn = 1
