...
with cte as(
select *,
abs(quantity*unitprice) as Sales_Amount,
ROW_NUMBER() over(partition by format(invoicedate, 'yyyy-MM') order by abs(quantity*unitprice) desc) rn
from walmart_eu
)
select format(invoicedate, 'yyyy-MM') as month, description, Sales_Amount as "Amount Paid"
from cte where rn = 1;