...
select category,
round(COUNT(rr.return_id)/cast((select COUNT(*) from return_records) as float)*100, 2) as return_percentage
from amazon_products ap join return_records rr on rr.product_id = ap.product_id
group by category
order by 2 desc;
