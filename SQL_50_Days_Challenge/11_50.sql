...
*/
--Sol 1
select h1.state, h1.city, h1.street_address, h1.mkt_price from house_price h1 where mkt_price > 
(select AVG(mkt_price) from house_price h2 where h1.city = h2.city);
...
select hp.state,hp.city, hp.street_address,hp.mkt_price from cte c join house_price hp on hp.city = c.city
where hp.mkt_price > c.avg_price;
