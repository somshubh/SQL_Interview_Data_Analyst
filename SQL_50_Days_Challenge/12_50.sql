...
with cte as(
select *,
ROW_NUMBER() over(partition by country order by profits desc) profilable_flag
from forbes_global
)
select company,sector,industry,country,sales,profits  from cte where profilable_flag = 1;